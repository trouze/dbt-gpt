...
{% macro gpt__alter_column_type(relation,column_name,new_column_type) -%}
...
{% macro gpt__check_schema_exists(information_schema,schema) -%}
...
{% macro gpt__create_schema(relation) -%}
...
{% macro gpt__drop_relation(relation) -%}
...
{% macro gpt__drop_schema(relation) -%}
...
{% macro gpt__get_columns_in_relation(relation) -%}
...
{% macro gpt__list_relations_without_caching(schema_relation) -%}
'''creates a table of relations withough using local caching.'''
{% endmacro %}

{% macro gpt__list_schemas(database) -%}
...
{% macro gpt__rename_relation(from_relation, to_relation) -%}
...
{% macro gpt__truncate_relation(relation) -%}
